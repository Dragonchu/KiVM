...
static String toClassName(mirrorOop mirror) {
  if (mirror->isPrimitiveMirror()) {
    ...
    switch (target->getClassType()) {
      case ClassType::INSTANCE_CLASS:return L"L" + target->getName() + L";";
      case ClassType::OBJECT_ARRAY_CLASS:
      case ClassType::TYPE_ARRAY_CLASS:return target->getName();
...
static Method *resolveTargetMethod(InstanceKlass *klass, int refKind,
  ...
  switch (refKind) {
    case REF_invokeVirtual:return klass->getVirtualMethod(methodName, descriptor);
    case REF_invokeStatic:return klass->getThisClassMethod(methodName, descriptor);
    default:SHOULD_NOT_REACH_HERE_M("Work in progress");
  }
}
...
JAVA_NATIVE jobject Java_java_lang_invoke_MethodHandleNatives_resolve(JNIEnv *env, jclass mhn,
  ...
  switch (_klass->getClassType()) {
    case ClassType::INSTANCE_CLASS:klass = (InstanceKlass *) _klass;
      break;
    case ClassType::TYPE_ARRAY_CLASS:
    case ClassType::OBJECT_ARRAY_CLASS:klass = Global::_Object;
      break;
    default:SHOULD_NOT_REACH_HERE();
  }
