      ...
      switch (arg->getMarkOop()->getOopType()) {
        ...
        case oopType::PRIMITIVE_OOP: {
          ...
          switch (valueType) {
            ...
            default:PANIC("Unknown value type: %d", valueType);
              break;
          }
          break;
        }

        default:PANIC("Unknown oop type");
      }
    }
    ++localVariableIndex;
  });
...
bool JavaCall::fillArguments(const std::vector<ValueType> &argTypes, bool hasThis) {
  ...
  for (auto it = argTypes.rbegin(); it != argTypes.rend(); ++it) {
    ...
    switch (valueType) {
      case ValueType::INT:callingArgs.push_front(new intOopDesc(_stack->popInt()));
        break;
      case ValueType::LONG:callingArgs.push_front(new longOopDesc(_stack->popLong()));
        break;
      case ValueType::FLOAT:callingArgs.push_front(new floatOopDesc(_stack->popFloat()));
        break;
      case ValueType::DOUBLE:callingArgs.push_front(new doubleOopDesc(_stack->popDouble()));
        break;
      case ValueType::OBJECT:
      case ValueType::ARRAY:callingArgs.push_front(Resolver::javaOop(_stack->popReference()));
        break;
