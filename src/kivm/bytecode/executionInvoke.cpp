...
static JavaObject("MethodHandle") makeMethodHandle(JavaThread *thread, RuntimeConstantPool *rt,
  ...
  if (isBootstrap) {
    switch (refKind) {
      case REF_invokeStatic:
      case REF_newInvokeSpecial:break;
      default:SHOULD_NOT_REACH_HERE();
        return nullptr;
    }
  }
  ...
  switch (refKind) {
    ...
    case REF_getStatic:
    case REF_putField:
    case REF_putStatic:return makeFieldHandle(thread, rt, lookupObject, refKind, refIndex);

    case REF_invokeVirtual:
    case REF_invokeStatic:
    case REF_invokeSpecial:
    case REF_newInvokeSpecial:
    case REF_invokeInterface:return makeInvokeHandle(thread, rt, lookupObject, refKind, refIndex);
...
oop Execution::invokeDynamic(JavaThread *thread, InstanceKlass *klass,
  ...
  for (int i = 0; i < bootstrap.num_bootstrap_arguments; ++i) {
    ...
    switch (rt->getRawPool()[argIndex]->tag) {
      case CONSTANT_String:callSiteArgs.push_back(rt->getString(argIndex));
        break;
      case CONSTANT_Class:callSiteArgs.push_back(rt->getClass(argIndex)->getJavaMirror());
        break;
      case CONSTANT_Integer:callSiteArgs.push_back(new intOopDesc(rt->getInt(argIndex)));
        break;
      case CONSTANT_Float:callSiteArgs.push_back(new floatOopDesc(rt->getFloat(argIndex)));
        break;
      case CONSTANT_Long:callSiteArgs.push_back(new longOopDesc(rt->getLong(argIndex)));
        break;
      case CONSTANT_Double:callSiteArgs.push_back(new doubleOopDesc(rt->getDouble(argIndex)));
        break;
